...
func (hs *HTTPServer) GetDashboard(c *models.ReqContext) Response {
	...
	if err != nil {
		return Error(500, "检查仪表板是否由用户加星标时出错", err)
	}
	...
	if dash.FolderId > 0 {
		...
		if err := bus.Dispatch(&query); err != nil {
			return Error(500, "无法读取仪表板文件夹", err)
		}
	...
	if err != nil {
		return Error(500, "检查仪表板是否已就绪时出错", err)
	}
...
func getDashboardHelper(orgID int64, slug string, id int64, uid string) (*models.Dashboard, Response) {
	...
	if err := bus.Dispatch(&query); err != nil {
		return nil, Error(404, "找不到仪表板", err)
	}
...
func DeleteDashboardBySlug(c *models.ReqContext) Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return Error(500, "无法通过slug获取仪表盘", err)
	}
...
func deleteDashboard(c *models.ReqContext) Response {
	...
	return JSON(200, util.DynMap{
		"title":   dash.Title,
		"message": fmt.Sprintf("仪表板 %s 已删除", dash.Title),
		"id":      dash.Id,
	})
}
...
func dashboardSaveErrorToApiResponse(err error) Response {
	...
	if ok := errors.As(err, &pluginErr); ok {
		message := fmt.Sprintf("此仪表板属于插件 %s.", pluginErr.PluginId)
		// look up plugin name
		if pluginDef, exist := plugins.Plugins[pluginErr.PluginId]; exist {
			message = fmt.Sprintf("此仪表板属于插件 %s.", pluginDef.Name)
		}
...
func (hs *HTTPServer) GetHomeDashboard(c *models.ReqContext) Response {
	...
	if prefsQuery.Result.HomeDashboardId != 0 {
		...
		log.Warnf("无法从数据库获取滑块, %s", err.Error())
	}
	...
	if err != nil {
		return Error(500, "无法加载首页仪表板", err)
	}
	defer file.Close()
...
func GetDashboardVersions(c *models.ReqContext) Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return Error(404, fmt.Sprintf("找不到仪表板Id %d 的版本", dashID), err)
	}

	for _, version := range query.Result {
		if version.RestoredFrom == version.Version {
			version.Message = "初始保存 (通过迁移创建)"
			continue
		}

		if version.RestoredFrom > 0 {
			version.Message = fmt.Sprintf("已从版本 %d 还原", version.RestoredFrom)
			continue
		}

		if version.ParentVersion == 0 {
			version.Message = "初始化保存"
		}
	}
...
func GetDashboardVersion(c *models.ReqContext) Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return Error(500, fmt.Sprintf("找不到版本为 %d 的仪表板 - dashboardId %d", query.Version, dashID), err)
	}
...
func CalculateDashboardDiff(c *models.ReqContext, apiOptions dtos.CalculateDiffOptions) Response {
	...
	if err != nil {
		if err == models.ErrDashboardVersionNotFound {
			return Error(404, "找不到仪表板版本", err)
		}
		return Error(500, "无法进行 diff 计算", err)
	}
...
func (hs *HTTPServer) RestoreDashboardVersion(c *models.ReqContext, apiCmd dtos.RestoreDashboardVersionCommand) Response {
	...
	saveCmd.Message = fmt.Sprintf("已从 %d 版本还原", version.Version)
...
func GetDashboardTags(c *models.ReqContext) {
	...
	err := bus.Dispatch(&query)
	if err != nil {
		c.JsonApiErr(500, "从数据库获取标签失败", err)
		return
	}
